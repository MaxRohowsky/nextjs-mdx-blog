...
- `sticky`: Changes the position value between `relative` and `fixed` depending on the scroll position.
  ...
  {<span className='h-52 block overflow-y-auto border'>
    ...
    <span className='sticky block border my-4  mx-2 bg-white bg-blue-100 dark:bg-black rounded-sm p-2 top-0'><p>This <code>span</code> with <code>position: sticky</code> stays at the top.</p></span>
