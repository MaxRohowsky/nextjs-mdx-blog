...
export default function Card({ item }: { item: Item }) {
    ...
    const img = 'img' in item ? item.img : '/sampleimg.webp';

    return (
        <Tilt tiltMaxAngleX={5} tiltMaxAngleY={5}>
            <div className="rounded-sm w-full h-full group cursor-pointer ">
                <a href={link} onClick={() => console.log(link)} className="flex flex-col relative justify-between border  dark:bg-background  border-gray-200 dark:border-neutral-800 rounded-lg hover:shadow-md transition-shadow duration-300 py-3 p-3 md:p-6 w-full h-full">
