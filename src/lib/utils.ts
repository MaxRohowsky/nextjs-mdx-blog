...
export const pathToBlogPosts = "src/app/blog/_posts";
export const pathToRemoteProjects = "src/app/projects/_remote/projects";
export const pathToLocalProjects = "src/app/projects/local/projects";
...
export function getFilteredProjects(options: Filter = {}): ProjectItem[] {
  ...
  const filteredProjects = allProjects.filter((project) => {
    if (
      ...
      !options.tags.every((tag) => project.tags.includes(tag))
    ) {
      return false;
    }
 ...
 */
export function getFilteredBlogItems(
  options: Filter = { isPublished: true },
  blogItems: FrontMatter[]
): FrontMatter[] {
  const filteredFrontMatter = blogItems.filter((item) => {
    ...
    if (
      ...
      !options.tags.every((optionTag) =>
        item.tags
          .map((tag) => tag.toLowerCase())
          .includes(optionTag.toLowerCase())
      )
    ) {
      return false;
    }
