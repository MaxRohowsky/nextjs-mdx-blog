...
title: Display and Position in CSS
...
- `static`: This is the default position value for all elements. They are not affected by top, bottom, left, and right properties.
  ...
  <span className='static block border rounded-sm px-2 dark:bg-black'>
...
Contrary to static, the top, bottom, left, and right properties do have an influence.
  ...
  <span className='relative block border rounded-sm px-2 right-10 dark:bg-black'>
...
- `fixed`: The positioning happens relative to the viewport, so the element stays in the same place even if you scroll
  ...
  <span className='fixed block border bg-white dark:bg-black rounded-sm px-2 bottom-0 z-10  '>
...
- `absolute`: Takes an element out of the normal document flow and positions it relative to its nearest 
  ...
  {<span className='relative block border bg-white dark:bg-black rounded-sm px-2 h-40 overflow-y-auto'>
    <p>
    This parent <code>span</code> has <code>position: relative</code> and the <span className="text-blue-400 font-bold">blue </span> 
    child <code>span</code> has <code>position: absolute</code>, <code>bottom: 0px</code>, and <code>right: 0px</code>.
    </p>

    <span className='absolute flex justify-center items-center border rounded-sm bottom-0 right-0 bg-blue-400 text-white w-12 h-12'>😎</span>
  </span>}
...
- `sticky`: Changes the position value between `relative` and `fixed` depending on the scroll position.
  ...
  {<span className='h-52 block overflow-y-auto border'>
    <span className='h-6 block px-2'><p>Scroll down on this element to see the sticky effect.</p></span>
    <span className='sticky block border my-4  mx-2 bg-white dark:bg-black rounded-sm p-2 top-0'><p>This <code>span</code> with <code>position: sticky</code> stays at the top.</p></span>
